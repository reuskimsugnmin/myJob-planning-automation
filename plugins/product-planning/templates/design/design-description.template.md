...
### 1. 뒤로 가기  <!-- 헤더 내비 = 배지 1, 정본 annotation-frame(clone). 별도 텍스트 블록 금지. SECTION 백/X 버튼에 배지 1도 함께. 기존 콘텐츠 배지는 +1 재번호 -->
header 좌상단 백버튼(/X). 이전 화면으로 돌아간다.
- [Action]
  - ⬅ 뒤로 → {{BACK_TARGET}} <!-- 예: P-01 상품 선택 -->
  - ✕ 닫기 → {{CLOSE_TARGET}} <!-- 있을 때만 -->
<!-- 헤더 없음/종료 화면: 본문 "종료 화면 · 헤더(뒤로/닫기) 미노출 · 이동은 하단 버튼" -->


<!-- 오버레이 상태 화면(셀렉트/안내 시트·팝업, figma-design §M-3/4/5): 본문에 [트리거](어디서 열리나) + [Action] 복귀 경로(선택/확인→반영 후 닫힘 / 취소·✕·바깥탭→원화면) 명시.
     안내 문구 = 인포박스(§M-6) variant: red=부정/주의 · blue=긍정 · gray=중립 안내. 결과 상세 데이터 = ui/detail(§M-7). -->

## 뱃지별 annotation  <!-- 배지 1 = 위 헤더 내비(뒤로 가기). 콘텐츠 요소는 배지 2부터 -->
> 제목 = 요소명. 본문 = (a) 역할 문장(라벨 없이) + (b) [대괄호] 섹션.
> ★ 값의 재료 = PRD/policy-table(화면만 보고 추측 금지). 각 필드는 FR/US/수용기준에서 옮기고 `(FR/US/P-XX)` 인용, 없으면 `[미확정 D-n]`.
> ★ 인터랙티브 요소는 종류별 **필수 필드**(스킬 §4-b1)를 작성 시점에 채운다(인터랙티브인데 [대괄호] 2개 미만 = 얕음 §5 B-4):
>   입력=[데이터]+[Validation]+[Default 상태]+[에러 케이스] · 버튼/CTA=[Default 상태]+[인터랙션]+[전이]+[엣지/예외] · 탭·칩=[상태]+[인터랙션] · 카드=[인터랙션]+[데이터] · 토글/체크/라디오=[Default 상태]+[인터랙션] · 셀렉트=[Default 상태]+[인터랙션]+[데이터] · 정적 라벨/아이콘=면제(역할 문장만).
...
<!-- ▼ --dev-detailed(기본): 개발 착수용 섹션. 부록이 아니라 기본 작성 요건 — 요소 종류별 필수(§4-b1)를 작성 시점에 채운다(--standard에서만 생략). -->
- [상태] <!-- 모든 상태 + 전이 트리거 -->
