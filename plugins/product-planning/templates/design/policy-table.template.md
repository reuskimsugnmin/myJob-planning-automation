...
- **의사결정**: {{DECISION_POINTER}} <!-- 전략적 UX 배치 등 사람 결정 항목은 인라인 단정 금지 → 하단 디자인 의사결정 체크리스트 D-n 포인터(예: → D-3). 제품 전략급이면 → PRD D-n -->
...
<!-- decision-checklist 스킬 스키마(기획 PRD C1과 동일). 디자인 단계 전략 결정(UX 배치·차별화·디자인 정책)을 여기 적재한다.
     ...
     제품 전략급(핵심밸류·타겟·서비스정책) 항목은 영향 범위에 → PRD D-n 으로 cross-link(PRD.md는 디자인 단계가 수정하지 않음). -->
