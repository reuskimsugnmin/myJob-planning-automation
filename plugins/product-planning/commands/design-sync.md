...
argument-hint: [engagement-slug] [--figma-url url] [--fidelity mid|high]
---
...
**4) fidelity 레벨(opt-in)** (`design-finalize` fidelity 레벨): `--fidelity high`(기본 `mid`)면 구조 검수 뒤 **high 추가 패스**(콘텐츠·머천다이징·일러스트·강조·구조 적정성 리포트 + illustrative 누출 검사 + 사람 잔여 punch-list)를 실행합니다. **전부 additive·제안·미리보기** — 매칭 컴포넌트·사람 수작업 보존, 시각 변경은 dry-run 보고 후 승인. `mid`/미지정은 기존 검증만.
