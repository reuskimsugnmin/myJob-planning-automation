...
- **대상**: 에러(검증 실패·인증 거절), 빈 상태(empty), 입력 완료(valid/활성), 처리중(loading), 결과 분기(성공/실패). **미세 상태(focus/hover/낱개 typing)는 화면화하지 않고 컴포넌트 variant**로 표현(`figma-design` §J).
- **절차**: 각 화면 디스크립션의 `[상태]`/`[엣지·예외]`/`[연동] 분기`에서 후보 케이스를 뽑아 **목록을 사용자에게 제시·확인**(과잉 생성 방지) → 승인분만 제작.
