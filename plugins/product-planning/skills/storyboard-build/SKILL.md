...
### 6. 디스크립션 → 디자인 역반영 고도화 (디스크립션 후 · 필수 HITL)
디스크립션을 쓰다 보면 해피패스 디자인에 **없던 케이스가 드러난다**(에러·empty·시트·팝업·disabled·validation·누락 안내). 이를 **텍스트로만 두지 말고 디자인으로 역반영**한다 — 두 레벨로:
- **요소 레벨(놓치기 쉬움)**: `[Default 상태]`·`[Validation]`·`[컴포넌트 상태]`가 가리키는 **컴포넌트 variant 전환**(버튼 disabled, 입력 error/typing, 탭 selected)과 **누락 요소 추가**(안내 인포박스 M-6·helper text·뱃지). 진입 상태(보통 Disabled)를 기본 렌더로(`figma-design` §J).
- **화면/오버레이 레벨**(아래) — 개발·QA가 별도 화면 없이는 모르는 **의미있는 분기만**:
