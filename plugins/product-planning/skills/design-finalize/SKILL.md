...
8. **컨테이너 적정성(F-6) 휴리스틱** — 각 화면이 ① 콘텐츠 밀도 낮음(빈 여백 비율 임계 초과) + ② 글랜스+단일/제로 액션 + ③ 부모 화면 즉시 트리거면 **"바텀시트/인라인 후보" 플래그**(재구성 방법은 `figma-design §M`). **★ 제외(오탐 방지): 리스트·메뉴·탭/그리드 컨테이너 화면은 데이터가 적어 저밀도여도 정상 → 플래그 금지.** 저밀도가 "데이터/빈상태 부재" 때문이면 F-6(시트)가 아니라 **F-1(콘텐츠)·빈상태 보강** 대상이다(실사례: eSIM e14·e15·e16·e09는 리스트/메뉴라 시트 아님 — `e11`만 글랜스+단일액션 시트). **자동 변환 금지 — 제안·미리보기만, 채택은 사람(⛳).**
