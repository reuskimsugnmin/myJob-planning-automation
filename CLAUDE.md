...
- 🔄 **Phase 3.5 (진행중, v0.9.17) — high-fi fidelity 고도화 (opt-in `--fidelity high`)**: toolset이 구조(mid)는 잘 짓지만 **콘텐츠·강조·머천다이징·일러스트·구조 적정성**은 못 채우던 갭(동료 high-fi 검토 기반 F-1~F-6)을 보강. **Phase 1 구현(v0.9.17)**: `design-finalize`에 fidelity 루브릭(low/mid/high) + high 추가 패스(차원 리포트·**illustrative 누출 검사**·**사람 잔여 punch-list**) + 컨테이너 적정성(F-6) 휴리스틱 훅; **F-1 콘텐츠 레인**(`content-kit` 템플릿 + `storyboard-build §3` 채움 규약 + `design-description` 예시값 표기, `provenance: spec|illustrative` 분리·개발계약 누출 0); `/storyboard`·`/design-sync` `--fidelity` 파라미터. **기본 `mid`=기존 동작 불변(opt-in)**, high는 마감 1~5 뒤 additive 패스. 이후 Phase 2(F-2 머천다이징)·2.5(F-6 구조 적정성 본구현)·3(F-4 강조·F-3 일러스트)·4(루브릭 스크립트+eSIM baseline) 예정. **high-fi 최종 확정(취향·단계 13)은 설계상 사람 몫**(도구는 바닥만 올리고 잔여는 punch-list).
